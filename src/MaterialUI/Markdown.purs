...
markdown source = RM.markdown
  { source
  , renderers:
    { heading: statelessComponent \ {level,children} -> unsafePartial $ case level of
    ...
    , blockquote: statelessComponent \ {children} -> typography {variant: caption}
      [ R.blockquote [] (childrenToArray children) ]
    , list: statelessComponent \ {children,ordered} ->
      typography {variant: body1}
      [ if ordered
          then R.ol [] (childrenToArray children)
          else R.ul [] (childrenToArray children)
      ]
    , inlineCode: statelessComponent \ {children} -> R.code [RP.style {border: "1px solid #ddd"}] (childrenToArray children)
    , code: statelessComponent \ {language,value} ->
      let codeProps = case toMaybe language of
            Nothing -> []
            Just l -> [RP.className ("language-" <> l)]
          code = R.code codeProps (childrenToArray value)
      in  R.pre [RP.style {border: "1px solid #ddd"}] [code]
    }
  }
