...
newtype WithSpanOfTime a = WithSpanOfTime
...
newtype ExploreTimeSpaces = ExploreTimeSpaces
  { name :: String
...
newtype ExploreTimeSpacesWithAux = ExploreTimeSpacesWithAux
  { name :: String
  , children :: Maybe
    { open :: Boolean
...
newExploreTimeSpacesSignal =
  IxSig.make $ WithSpanOfTime
    { spanOfTime: SpanOfTime
      { startTime: "1234"
      , stopTime: "5678"
      }
    , timeSpaces: ExploreTimeSpaces
      { name: "TimeSpace Name"
      , children:
        let {set} = IxDemiSet.fromFoldable
              [ Tuple (SpanOfTime {startTime: "a", stopTime: "b"}) $ ExploreTimeSpaces
                {name: "TimeSpace Child 1", children: IxDemiSet.empty}
              , Tuple (SpanOfTime {startTime: "c", stopTime: "d"}) $ ExploreTimeSpaces
                {name: "TimeSpace Child 2", children:
                  let {set} = IxDemiSet.fromFoldable
                        [ Tuple (SpanOfTime {startTime: "g", stopTime: "h"}) $ ExploreTimeSpaces
                          {name: "TimeSpace GrandChild 1", children: IxDemiSet.empty}
                        , Tuple (SpanOfTime {startTime: "i", stopTime: "j"}) $ ExploreTimeSpaces
                          {name: "TimeSpace GrandChild 2", children: IxDemiSet.empty}
                        ]
                  in  set
                }
              , Tuple (SpanOfTime {startTime: "e", stopTime: "f"}) $ ExploreTimeSpaces
...
updateExploreTimeSpacesWithAux (ExploreTimeSpacesWithAux x) (ExploreTimeSpaces y) = ExploreTimeSpacesWithAux
  x { name = y.name
    , children =
      if IxDemiSet.size y.children == 0
        then Nothing
        else Just $ case x.children of
          Nothing ->
            { open: false
...
toggleOpen index orig@(ExploreTimeSpacesWithAux x) = case x.children of
  ...
  Just y -> case Array.uncons index of
    ...
    Just {head,tail} -> case IxDemiSet.lookup head y.children of
      Nothing -> orig
      Just {value} -> ExploreTimeSpacesWithAux x
        { children = Just y
...
openThrough is ts =
  let iss :: Array (Array Index)
      iss =
        let basis = [[]]
            go :: Array (Array Index) -> Index -> _
            go acc i = unsafePartial $ case Array.unsnoc acc of
              Just {init,last} -> Array.snoc acc (Array.snoc last i)
        in  Array.foldl go basis is
  in  Array.foldr (setOpen true) ts iss
