...
onNew { dangerConfirmQueues, timeScaleSignal, timeSpaceNameSignal } =
  launchAff_ do
    resetAll <- IOQueues.callAsync dangerConfirmQueues $ DangerConfirm
      { content:
        [ typography
            { variant: body1
            }
...
onClickedNewTimeline {newOrEditTimelineQueues, timelinesSignal} = launchAff_ do
  mResult <- IOQueues.callAsync newOrEditTimelineQueues Nothing
  case mResult of
    Nothing -> pure unit
    Just result -> case result of
      NewOrEditTimeline t -> liftEffect do
...
onClickedEditTimeline {newOrEditTimelineQueues, timelinesSignal} index = do
  ...
  case timelines Array.!! index of
    ...
    Just t -> launchAff_ do
      ...
      case mResult of
        Nothing -> pure unit
        Just result -> liftEffect $ case result of
...
onClickedDeleteTimeline {dangerConfirmQueues,timelinesSignal} eIndexAuxAction = launchAff_ do
  confirmed <- IOQueues.callAsync dangerConfirmQueues $ DangerConfirm
    { content:
  ...
  when confirmed $ liftEffect $ case eIndexAuxAction of
    Left index -> do
      Timelines timelines <- IxSig.get timelinesSignal
